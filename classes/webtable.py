class WebTable:
    ...
    def row_data(self, row_number):
        ...
        row = self.table.find_elements_by_xpath(f'//tr[{str(row_number)}]/td')
        r_data = []
    ...
    def column_data(self, column_number):
        col = self.table.find_elements_by_xpath(f'//tr/td[{str(column_number)}]')
        r_data = []
    ...
    def get_all_data(self):
        ...
        for i in range(2, num_rows):
            ro = ["**(" + self.table.find_element_by_xpath(f'//tr[{str(i)}]/th').text + ")**"]
            num_columns = len(self.table.find_elements_by_xpath(f'//tr[{i}]/td')) + 1
            for j in range(1, num_columns):
                current = self.table.find_element_by_xpath(f'//tr[{str(i)}]/td[{str(j)}]').text
                if current:
                    ro.append(current)
    ...
    def get_cell_data(self, row_number, column_number):
        if row_number == 0:
            raise Exception("Row number starts from 1")
        row_number = row_number + 1
        cell_data = self.table.find_element_by_xpath(f'//tr[{str(row_number)}]/td[{str(column_number)}]').text
        return cell_data
