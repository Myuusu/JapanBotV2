...
class Math(commands.Cog):
    ...
    async def floor(self, ctx, term: str):
        if term:
            term = await clean_float(term)
            await ctx.send(await trim(f'floor({term}) = {math.floor(term):,}'))
        else:
    ...
    async def factorial(self, ctx, term: str):
        if term:
            term = await clean_float(term)
            await ctx.send(await trim(f'{term}! = {math.factorial(term):,}'))
        else:
    ...
    async def product(self, ctx, term1: str, term2: str):
        if term1 and term2:
            term1 = await clean_float(term1)
            term2 = await clean_float(term2)
            await ctx.send(await trim(f'{term1} * {term2} = {term1*term2:,}'))
        else:
    ...
    async def sqrt(self, ctx, term: str):
        if term:
            term = await clean_float(term)
            await ctx.send(await trim(f'sqrt({term}) = {math.sqrt(term):,}'))
        else:
            await ctx.send('Please specify one term to produce a square root!')
    ...
    async def pow(self, ctx, term1: str, term2: str = 2):
        if term1 and term2:
            term1 = await clean_float(term1)
            term2 = await clean_float(term2)
            await ctx.send(await trim(f'{term1}^{term2} = {math.pow(term1,term2):,}'))
        else:
            await ctx.send('Please specify two term to produce a !')
    ...
    async def subtraction(self, ctx, term1: str, term2: str):
        if term1 and term2:
            term1 = await clean_float(term1)
            term2 = await clean_float(term2)
            await ctx.send(await trim(f'{term1} - {term2} = {term1-term2:,.0f}'))
        else:
            await ctx.send('Please specify two terms to produce a quotient!')
