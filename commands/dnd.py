...
from storage.dnd_list import class_list, race_list, skill_list
...
class DND(commands.Cog):
    ...
    # !create_character Vol Fighter 5 Soldier "Chaotic Neutral" Human 900 acrobatics,survival 10,11,12,13,14,15
    ...
    async def attributes(self, ctx, *, attributes=None):
        if attributes is None:
            ...
            try:
                ...
                await msg.edit(content='Timed Out. Please reissue command.', delete_after=10)
        else:
    ...
    async def proficiencies(self, ctx, *, skills=None):
        if skills is None:
            msg = await ctx.send(embed=self.skill_embed)
    ...
    async def race(self, ctx, *, race=None):
        if race is None:
            msg = await ctx.send(embed=self.race_embed)
    ...
    async def class_type(self, ctx, *, class_type=None):
        if class_type is None:
            msg = await ctx.send(embed=self.class_type_embed)
